...
class Solution:
    ...
    def generate_combinations(self, current: str, remaining_numbers: list[str]) -> list[str]:
        if not remaining_numbers:
            return [current]
        next_number, *rest = remaining_numbers
        sum_expr = f"({current} + {next_number})"
        prod_expr = f"({current} * {next_number})"
        return self.generate_combinations(sum_expr, rest) + self.generate_combinations(prod_expr, rest)
    ...
    def extract_valid_test_values(self, line: str, pattern: re.Pattern[str]) -> str:
        ...
        combos_results = [eval(combo) for combo in self.generate_combinations(numbers[0], numbers[1:])]
        return test_value if combos_results.count(int(test_value)) else "0"

    def first_task(self) -> int:
        pattern = re.compile(r"\d+")
        valid_test_values = parse_all(self.lines, self.extract_valid_test_values, pattern)
