...
class Solution:
    ...
    def get_all_antinodes(grid: Grid, left: Point, right: Point) -> list[Point]:
        ...
        above = [Point(*right)]
        candidate_above = Point(*[coord_r - coord_d for coord_r, coord_d in zip(right, delta)])
        while grid.is_inside(candidate_above):
            above.append(candidate_above)
            candidate_above = Point(*[coord_r - coord_d for coord_r, coord_d in zip(candidate_above, delta)])
        below = [Point(*left)]
        candidate_below = Point(*[coord_l + coord_d for coord_l, coord_d in zip(left, delta)])
        while grid.is_inside(candidate_below):
            below.append(candidate_below)
            candidate_below = Point(*[coord_l + coord_d for coord_l, coord_d in zip(candidate_below, delta)])
        return above + below
